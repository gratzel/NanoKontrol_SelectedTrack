...
#	__module__ = __name__
	...
	def do_nothing(self, value, mode, status):
		pass
...
#			param.value = max(0.0, min(1.0, param.value + (value/200.0)))
	...
	def switch_monitoring(self, value, mode, status):
		...
		if (hasattr(track, "current_monitoring_state")):
			track.current_monitoring_state = (track.current_monitoring_state + 1) % (len(track.monitoring_states.values)-1)
