...
#	__module__ = __name__
	...
	def get_midi_bindings(self):
		return (
			...
			("force_clip_loop", self.force_clip_loop),
			("clip_gain", self.clip_gain),
		)
	
	def clip_gain(self, value, mode, status):
		clip = self.song.view.detail_clip
		clip.gain = float(value/127.0)
	
	def force_clip_loop(self, value, mode, status):
		clip = self.song.view.detail_clip
		#log("clip = " + str(dir(clip)))
		clip.looping = 1
