...
class MIDICommand:
	def __init__(self, key, mode = ABSOLUTE, status = NOTEON_STATUS, channel = DEFAULT_CHANNEL, shift = 0):
		self.key = key + shift
		self.mode = mode
		self.status = status
		self.channel = channel
class Note (MIDICommand):
	def __init__(self, note, shift=0, channel = DEFAULT_CHANNEL):
		MIDICommand.__init__(self, note, ABSOLUTE, NOTEON_STATUS, channel, shift)
...
class PB (MIDICommand):
	def __init__(self, pb, mode = ABSOLUTE, channel = DEFAULT_CHANNEL):
		# we'll set both the channel and the key to the specified argument
		# i.e., when specifying PB(x), x will set both channel and key
		MIDICommand.__init__(self, key = pb, mode = mode, status = PB_STATUS, channel = pb)		